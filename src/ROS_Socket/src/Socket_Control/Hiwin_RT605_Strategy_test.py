...
temp_label = ""
is_metal = 0  #count times that camera see
is_bottle = 0
is_paper = 0
...
def realsense_look():      #get what realsense see (depth need to fix)
    ...
    # if switch_flag == 0 and cntr_point.center_point_x < -10 or cntr_point.center_point_x > 18.5 or cntr_point.center_point_y < 46.5 or cntr_point.center_point_y > 63.5:   #edit box edge x & y
    #     cntr_point.center_point_x = 4.5
    #     cntr_point.center_point_y = 55.5
    #     print("no I think maybe zed will do better!")
    #     switch_flag = 1
...
def call_get_point(input_key):  #for zed calculate
    try:
...
def identify_item():
    global item_name,diff_item,is_bottle,is_metal,is_paper
    if item_name != None:
        if is_bottle > is_metal and is_bottle > is_paper:
            diff_item = 0
        elif is_paper > is_metal and is_paper > is_bottle:
            diff_item = 1
        elif is_metal > is_bottle and is_metal > is_paper:
            diff_item = 2    
    else:
        diff_item = 3
...
def callback_realsense(data):     #get arm orientation(but need to reverse pitch)
#    global realsense_point
    realsense_point.roll = data.pose.orientation.x
    realsense_point.pitch = data.pose.orientation.y -180
    realsense_point.yaw = data.pose.orientation.z
...
def yolo_receive():  #print what yolo see
    global temp_label,temp_bottle,temp_metal,temp_paper
    cant_see_flag = 1
...
def depth_callback(depth_data):
    global Depth_Image
    DI = Depth_Bridge.imgmsg_to_cv2(depth_data, "32FC1")
