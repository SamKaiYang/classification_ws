...
def Get_MissionType():
    ...
    for case in switch(MissionType_Flag): #傳送指令給socket選擇手臂動作
        if case(0):
            ...
            Missionstrategy_client_Arm_Mode
            break
        if case(1):strategy_client_Arm_Mode
...
def MotionItem(ItemNo):
    ...
    if MoveFlag == True:
        if LinePtpFlag == False:
            ...
            ArmTask.strategy_client_pos_move(pos.x,pos.y,pos.z,pos.pitch,pos.roll,pos.yaw)
    #action: pstrategy_client_Arm_Mode
    #ra : abs strategy_client_Arm_Mode
    #grip 夾爪strategy_client_Arm_Mode
    #vel speedstrategy_client_Arm_Mode
    #both : Ctrl_Mode
##-------------strategy end ------------
def myhook():
    print ("shutdown time!")
