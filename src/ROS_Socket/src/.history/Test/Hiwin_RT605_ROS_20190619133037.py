...
class switch(object):
    ...
    def match(self, *args):
        ...
        if self.fall or not args:
            ...
            return False
class Arm_state_flag():
    def __init__(self,Arm):
        self.Arm = Arm
        #self.Strategy = Strategy
    def Arm_feedback(self):
        return int(self.Arm)
    #def Strategy_feedback(self):
    #    return int(self.Strategy)
class Strategy_state_flag():
...
def Arm_state(req):
    ...
    print(Arm_state_flag(int('%s'%req.Arm_state)).Arm_feedback)
    if Arm_state_flag.Arm_feedback  == 1: #表示手臂忙碌
        Strategy_state_flag(0)
        return(1)
    if Arm_state_flag.Arm_feedback  == 0: #表示手臂閒置
        Strategy_state_flag(1)
        print(1111)
        return(0)
    if Arm_state_flag.Arm_feedback == 6: #表示程式中斷
        Strategy_state_flag(6)
        return(6)
def strategy_server():
