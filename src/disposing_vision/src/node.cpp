...
ros::Publisher pub_normal,pub_pose,pub_procced,pub_pose_pry;
...
geometry_msgs::PoseStamped object_rpy;
...
void zed_cloud_cb(const sensor_msgs::Image::ConstPtr& msg)
{
...
//------------------------------------------------------------------------------
  ...
  Pcl_function.passthrough(object_cloud,object_cloud,"y",y_coordinate_min,y_coordinate_Max);
  Pcl_function.passthrough(object_cloud,object_cloud,"z",z_coordinate_min,z_coordinate_Max);
  // printf("x_coordinate_min,x_coordinate_Max = %f,%f\n",x_coordinate_min, x_coordinate_Max);
  // printf("y_coordinate_min,y_coordinate_Max = %f,%f\n",y_coordinate_min, y_coordinate_Max);
  // printf("z_coordinate_min,z_coordinate_Max = %f,%f\n",z_coordinate_min, z_coordinate_Max);

  Pcl_function.downsampling(object_cloud,object_cloud,0.0025);
  ...
  // writer.write ("/home/luca/catkin_ws/src/disposing_vision/src/object.pcd", *object_cloud, true);
  // writer.write ("/home/luca/catkin_ws/src/disposing_vision/src/plane.pcd", *plane_cloud, true);
  ...
  //=================
  object_rpy.header.frame_id = "zed_left_camera_frame";
  object_rpy.header.stamp = ros::Time::now();;
  object_rpy.header.seq = 2;
  //=================
  ...
  object_rpy.pose.orientation = msg;
  object_rpy.pose.position.x = object_normal.x;
  object_rpy.pose.position.y = object_normal.y;
  object_rpy.pose.position.z = object_normal.z;
  ...
  if(Use_Zed)
  {
    object_rpy.pose.orientation.x =  -1.0*(pitch);
    object_rpy.pose.orientation.y = -1.0*(yaw);
    object_rpy.pose.orientation.z = roll;
    std::cout<<"use zed"<<std::endl;
  }
  else
  {
    object_rpy.pose.orientation.x = roll;
    object_rpy.pose.orientation.y = pitch;
    object_rpy.pose.orientation.z = yaw;
  }
  
  // std::cout<<"roll, pitch, yaw: "<<roll<<", "<<pitch<<", "<<yaw<<endl;
  pub_pose_pry.publish(object_rpy);
  std::cout << "\tTotal Normal:" << object_rpy.pose.orientation.x << " "
                                << object_rpy.pose.orientation.y << " "
                                << object_rpy.pose.orientation.z << " "
                                << std::endl;
...
void set_zed_coordinate_limit_min(const geometry_msgs::Point& input)
{
  y_coordinate_Max = -1*input.x;
  z_coordinate_Max = -1*input.y;
  x_coordinate_min = input.z;
}

void set_zed_coordinate_limit_Max(const geometry_msgs::Point& input)
{
  y_coordinate_min = -1*input.x;
  z_coordinate_min = -1*input.y;
  x_coordinate_Max = input.z;
}
...
int main (int argc, char** argv)
{
  ...
  // ros::Subscriber sub = nh.subscribe ("/zed/zed_node/depth/depth_registered", 10, zed_cloud_cb);
  ...
  ros::Subscriber sub_coordinate_min = nh.subscribe ("/coordinate_limit_min", 1, set_zed_coordinate_limit_min);
  ros::Subscriber sub_coordinate_Max = nh.subscribe ("/coordinate_limit_Max", 1, set_zed_coordinate_limit_Max);

  // Create a ROS publisher for the output point cloud
  pub_pose = nh.advertise<geometry_msgs::PoseStamped> ("/object/pose", 1);
  pub_pose_pry = nh.advertise<geometry_msgs::PoseStamped> ("/object/pose_rpy", 1);
...
//   if ( pcl::io::loadPCDFile <pcl::PointXYZ> ("/home/luca/catkin_ws/src/disposing_vision/src/plane.pcd", *cloud) == -1)
//   {
