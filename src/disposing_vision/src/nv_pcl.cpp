...
mask_rcnn::coordinate_normal object_normal;
geometry_msgs::PoseStamped object_pose;

mask_rcnn::coordinate_normal average_normal(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud
                                                  ,pcl::PointCloud<pcl::PointNormal>::Ptr input_normal){

  mask_rcnn::coordinate_normal average_normal;
  average_normal.x=0;
  average_normal.y=0;
  average_normal.z=0;
  ...
  for (size_t currentPoint = 0; currentPoint < input_normal->points.size(); currentPoint++)
	{
    if(!(std::isnan(input_normal->points[currentPoint].normal[0])||
         std::isnan(input_normal->points[currentPoint].normal[1])||
         std::isnan(input_normal->points[currentPoint].normal[2])))
    {
...
//------------------------------------------------------------------------------
  ...
  //passthrough
  // Pcl_function.passthrough(cloud,object_cloud,"x", 0  , 0.001);
  // Pcl_function.passthrough(cloud,object_cloud,"y", 0  , 0.001);
  ...
  Pcl_function.passthrough(cloud,object_cloud,"z", 0.7, 1);
...
int main (int argc, char** argv)
{
  ...
  // ros::Subscriber sub = nh.subscribe ("/camera/depth_registered/points", 1, cloud_cb);    ros::Subscriber sub = nh.subscribe ("/camera/depth_registered/points", 1, cloud_cb);
  // ros::Subscriber sub = nh.subscribe ("/camera/depth/color/points", 1, cloud_cb);
  ros::Subscriber sub = nh.subscribe ("/camera/depth_registered/points", 1, cloud_cb);

  // 寫一個ros node 來 run 下面兩個 subscribe
  ...
  pub_normal = nh.advertise<mask_rcnn::coordinate_normal> ("/object/normal", 1);
